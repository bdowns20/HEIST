...
# Also designed to run without requiring any API keys or endpoints
...
st.info("""
**Demo Version:** This is a sharable demonstration of the HEIST simulation dashboard with simulated AI capabilities.
No API keys or configuration required. For the full application with all features and real API integration, please contact the development team.
""")

# Simulation parameters directly in the sidebar without API configuration
...
def generate_log_entry(cycle, network_state, threat_data=None):
    ...
    if threat_data and threat_data["threat_data"]["probability"] > high_alert_threshold:
        ...
        log += f"[SA Hub] AI warning: Potential {threat_data['threat_data']['type']} detected ({threat_data['threat_data']['probability']:.2f} probability)\n"
...
with tab1:
    ...
    st.write("No API keys required - all AI capabilities are simulated for demonstration purposes.")
